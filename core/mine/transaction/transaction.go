...
const (
	noncePrefix              = "/transaction/nonce/"
	storedTransactionPrefix  = "/transaction/stored/"
	pendingTransactionPrefix = "/transaction/pending/"
)
...
func NewService(backend Backend, signer crypto.Signer, store statestore.StateStore, chainID *big.Int, monitor Monitor) (Service, error) {
...
func (t *transactionService) Send(ctx context.Context, request *TxRequest) (txHash common.Hash, err error) {
	...
	log.Infof("sending transaction %x with nonce %d", signedTx.Hash(), nonce)
	...
	go func() {
		...
		if err != nil {
			if !errors.Is(err, ErrTransactionCancelled) {
				...
				log.Warnf("pending transaction %x cancelled", txHash)
			}
		} else {
			log.Infof("pending transaction %x confirmed", txHash)
		}
...
func (t *transactionService) nonceKey() datastore.Key {
	return datastore.NewKey(fmt.Sprintf("%s%x", noncePrefix, t.sender))
}

func storedTransactionKey(txHash common.Hash) datastore.Key {
	return datastore.NewKey(fmt.Sprintf("%s%x", storedTransactionPrefix, txHash))
}

func pendingTransactionKey(txHash common.Hash) datastore.Key {
	return datastore.NewKey(fmt.Sprintf("%s%x", pendingTransactionPrefix, txHash))
}
...
func (t *transactionService) nextNonce(ctx context.Context) (uint64, error) {
	...
	if err != nil {
		log.Errorf("nextNonce error: %v", err)
		return 0, err
	}
	...
	if err != nil {
		log.Errorf("store get none error: %v", err)
...
func (t *transactionService) PendingTransactions() ([]common.Hash, error) {
	...
	err := t.store.Iterate(pendingTransactionPrefix, func(key string, value []byte) (stop bool, err error) {
		txHash := common.HexToHash(strings.TrimPrefix(key, pendingTransactionPrefix))
